...
userRouter.patch('/:user_id/message',jwtMiddleware,stickerController.postMessage); //메세지 등록(Host, Visitor)
userRouter.get('/visitor_sticker/:visitor_sticker_id',stickerController.getSticker); //방문자 기록 visitor_sticker_id로 상세 조회 API
userRouter.get("/:user_id/sticker",jwtMiddleware,stickerController.getStickers); //방문자 기록 페이지 조회
...
userRouter.patch('/:user_id/sticker',jwtMiddleware,stickerController.attachSticker); //스티커 부착, 스티머 위치 수정도 같은 API 호출
